    ...
    def run_report():
        with psycopg2.connect(
            ...
            fig_rides_weather_ts.update_layout(
                width=1200, height=500, template="simple_white"
            )

            correls = [
                df_rides_by_day[[field, "n_rides"]].dropna().corr().values[0, 1]
                for field in ["daily_high", "precip_amount"]
            ]
            ...
            for col, y_label in enumerate(
                ...
                fig_rides_weather_relationship.update_layout(
                    **{axis_title_key: "Daily Rides"}
                )
                axis_title_key = f"yaxis{col+1}_title"
                fig_rides_weather_relationship.update_layout(
                    **{axis_title_key: y_label}
                )
            ...
            correls = [
                df_zip_codes[[field, "station_capacity"]].dropna().corr().values[0, 1]
                for field in ["total_returns", "avg_agi"]
            ]
            ...
            fig_zip_codes.update_layout(width=800, height=500, template="simple_white")
            ...
            with open(
                REPORTING_PATH
                / "reports"
                / f'report_{datetime.strftime(datetime.now(), "%Y%m%d%H%M")}.html',
                "w",
            ) as f:
                f.write(template)
