...
def map_lat_lon_data_to_zip(
    df_lat_lon: pd.DataFrame, df_zip: pd.DataFrame
) -> pd.Series:
...
def get_station_info_data():
    ...
    df_station_status["zip_code"] = pd.to_numeric(
        map_lat_lon_data_to_zip(df_station_status, df_zip), errors="coerce"
    ).apply(
        lambda x: f"{int(x):05d}" if pd.notna(x) and x != "inf" and x == x else None
    )
