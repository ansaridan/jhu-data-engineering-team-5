...
def drop_recreate_schema(conn: psql_ext.connection, schema: str) -> None:
    # drop all tables
    try:
...
def generate_create_table_statement(
    schema: str, table_name: str, schema_data: dict
) -> str:
...
def drop_recreate_table(
    *, db_schema: str, table_name: str, table_schema: dict, conn: psql_ext.connection
) -> None:
    try:
    ...
    try:
        with conn.cursor() as cursor:
            ...
            create_statement = generate_create_table_statement(
                db_schema, table_name, table_schema
            )
...
def upload_dataframe(
    *,
    ...
    query = "INSERT INTO {}.{} ({}) VALUES %s{}".format(
        db_schema, table_name, ",".join(columns), conflict_clause
    )
